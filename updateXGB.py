...
if __name__ == '__main__':
	...
	num_round = 5
	bs = xgb.Booster({'nthread':4})
	bst = xgb.Booster({'nthread':4})
	bst.load_model('xgb_pathClass.model')
	bstFin = xgb.Booster({'nthread':4})
	# bstFin.load_model('xgb_pathClass.model')
	...
	for key in qnID_dict:
		...
		# bs = xgb.train(param, dtrain, num_round, watchlist)
		# bs.save_model('model1.model')
		if(i == 1 ):
			bstFin = xgb.train(param, dtrain, num_round, watchlist, xgb_model = bst)
			bstFin.save_model('model2.model')
			print("i: ", i)# bst.update(dtrain, i)
		elif (i >= 2):
			bstFin = xgb.train(param, dtrain, num_round, watchlist, xgb_model = bstFin)
			bstFin.save_model('model3.model')
			print("i: ", i)

		i += 1

	bstFin.save_model('xgb_update.model')
